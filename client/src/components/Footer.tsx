...
const Footer = () => {
  ...
  const XIcon = () => (
    ...
    >
      <path
        fill="#90A1B9"
        d="M714.163 519.284L1160.89 0H1050.24L668.408 442.64 369.23 0H0l463.94 681.478L0 1226.37h110.65l404.77-476.7 319.393 476.7H1200L714.163 519.284ZM560.292 687.293l-46.774-67.486L150.38 80h157.07l303.02 437.774 46.774 67.487L1045.61 1147h-157.07L560.292 687.293Z"
      />
    </svg>
  );
  ...
  const linkSections = [
    ...
    },
    {
      ...
      links: [
        {
          text: "+234-805-555-6677",
          path: "tel:+2348055556677",
          icon: PhoneIcon,
        },
        {
          text: "contact@ngecp.com",
          path: "mailto:contact@ngecp.com",
          icon: MailIcon,
        },
  ...
  return (
    <footer className="mx-6 bg-gradient-to-b from-slate-50 to-white border-t border-slate-200/60 mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-start justify-between gap-12 py-16 border-b border-slate-200 text-slate-500">
          {/* Brand Info */}
          <div className="max-w-md">
            <Link
              to="/"
              className="text-4xl font-semibold text-slate-800 inline-block"
            >
              <span className="text-emerald-600">N</span>
              <span className="text-slate-800 font-bold">GECP</span>
              <span className="text-emerald-500 text-5xl leading-none">.</span>
            </Link>
            ...
            <div className="flex items-center gap-3 mt-6">
              {socialIcons.map((item, i) => (
                <a
                  ...
                  className="flex items-center justify-center w-10 h-10 bg-white hover:bg-slate-100 hover:scale-105 border border-slate-200 shadow-sm transition rounded-full text-slate-600"
                >
                  <item.icon />
                </a>
              ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 md:gap-12 w-full md:w-auto">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="font-bold text-xs tracking-wider text-slate-900 uppercase mb-4">
                  {section.title}
                </h3>
        ...
        {/* Copyright Notice */}
        <div className="py-6 text-xs sm:text-sm text-center text-slate-500">
